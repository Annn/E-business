...
class OrderController @Inject()(orderRepository: OrderRepository, cartRepository: CartRepository,
  ...
  def index = Action.async { implicit request =>
    ...
    carts.map(c => Ok(views.html.order(orderForm, c)))
  ...
  */
  ...
  def addOrder = Action.async { implicit request =>
    ...
    orderForm.bindFromRequest.fold(
      ...
      errorForm => {
        Future.successful(
          Ok(views.html.order(errorForm, a))
        )
      },
      // There were no errors in the from, so create the order.
      order => {
        orderRepository.create(order.cart, order.order_date, order.address, order.cost).map { _ =>
...
/**
  ...
  */
case class CreateOrderForm(cart: Int, order_date: String, address: String, cost: Int)